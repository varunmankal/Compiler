...
public class ScannerTest {
	...
	public void testString() throws LexicalException{
		...
		checkNext(scanner,INTEGER_LITERAL,11,3,2,1);
		checkNext(scanner,IDENTIFIER,14,3,2,4);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testString_1() throws LexicalException{
		String input="polar_a@DEF_X@DEF_YAb0;;;<";
		Scanner scanner=new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner,KW_polar_a,0,7,1,1 );
		checkNext(scanner,OP_AT,7,1,1,8);
		checkNext(scanner,KW_DEF_X,8,5,1,9);
		checkNext(scanner,OP_AT,13,1,1,14);
		checkNext(scanner,IDENTIFIER,14,8,1,15);
		checkNext(scanner,SEMI,22,1,1,23);
		checkNext(scanner,SEMI,23,1,1,24);
		checkNext(scanner,SEMI,24,1,1,25);
		checkNext(scanner,OP_LT,25,1,1,26);
		checkNextIsEOF(scanner);
	}
